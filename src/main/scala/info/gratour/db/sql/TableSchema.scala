...
import info.gratour.db.rest.{CustomFieldLoaderProvider, QueryParamsSpecBuilder}
import info.gratour.db.schema.{FieldConstraint, FieldDataType, FieldNameMapper}
import info.gratour.db.sql.UpsertBuilder._
...
case class TableSchema(
  ...
  def queryParamsBuilder[T >: Null <: AnyRef](select: String, entryClass: Class[T],
    ...
    new QueryParamsSpecBuilder(select, entryClass, resolver, mapper, customFieldLoaderProvider, groupByClause, cnArray)
  }
  ...
  def delete(keys: Any*)(implicit template: JdbcTemplate): Boolean = {
    ...
    template.update(DELETE_SQL, StatementSetter(binder => {
      keys.foreach(k =>
        binder.set(k))
      //      binder.bind(keys)
    })) > 0
...
class UpsertBuilder[T >: Null <: AnyRef](val tableSchema: TableSchema, val fieldsOfEntryClass: Array[Field]) {
  ...
  def updateValue(columnName: String, value: Object): UpsertBuilder[T] = {
    ...
    updateValues += UpsertBuilder.ColumnValue(columnName, GENERAL(value))
...
object UpsertBuilder {
  ...
  private[sql] case class FIELD_STUB(field: Field, column: ColumnDef) extends Value
