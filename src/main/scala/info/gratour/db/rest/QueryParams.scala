...
import info.gratour.common.types.{EpochMillis, Flag}
import info.gratour.common.utils.{DateTimeUtils, StringUtils}
import info.gratour.db.rest.QueryParams.HookedMapper
import info.gratour.db.schema.{FieldConstraint, FieldDataType, FieldNameMapper, PaginationSupportSpec, Predication}
import info.gratour.db.sql.{FieldResolver, MapperBuilder, QueryResult, QueryResultObjectLoader}
  ...
  def check(c: SearchCondition, dbColumnNames: Array[String]): ParsedSearchCondition = {
    ...
    if (pred1 == Predication.IS_NULL || pred1 == Predication.NOT_NULL) {
      ...
      dataType match {
        ...
        case FieldDataType.EPOCH_MILLIS =>
          for (s <- phase1ParseResult.values) {
            v = StringUtils.tryParseLong(s)
            if (v == null) {
              v = StringUtils.tryParseOffsetDateTime(s)
...
case class ParsedQueryParams[T <: AnyRef](
  ...
  def singleCondValueAsEpochMillis(paramName: String): EpochMillis = {
    ...
    conds(0).parsedValues(0) match {
      ...
      case ldt: LocalDateTime =>
        EpochMillis(ldt.toInstant(DateTimeUtils.defaultZoneOffset).toEpochMilli)
  ...
  def replaceSelect(newSelect: String): ParsedQueryParams[T] =
    ParsedQueryParams(spec, newSelect, conditions, pagination, sortColumns, fields, groupByClause)
    ...
    if (conditions != null && !conditions.isEmpty) {
      ...
      conditions.conditions.foreach(cond => {
        ...
        def value(index: Int): String = cond.spec.dataType match {
          ...
          case FieldDataType.OFFSET_DATETIME | FieldDataType.EPOCH_MILLIS =>
            val v = cond.parsedValues(index) match {
              case l: java.lang.Long =>
                OffsetDateTime.ofInstant(Instant.ofEpochMilli(l), Consts.ZONE_ID_Z)
              case ep: EpochMillis =>
                ep.toOffsetDateTimeZ
              case odt: OffsetDateTime =>
                odt
              case _ =>
        ...
        val expressions = cond.dbColumnNames.map(colName => {
          ...
          val cond1 = cond.predication match {
            ...
            case Predication.UNDER =>
              if (!cond.parsedValues(0).isInstanceOf[Number]) {
                throw ErrorWithCode.invalidParam(cond.paramName)
              }
              "p_is_grp_under(" + cond.parsedValues(0).asInstanceOf[Number].longValue() + "," + columnName + ")"
  ...
  def toSql: String = {
    ...
    if (pagination == null) {
      if (groupBy.nonEmpty)
        select + where.toString + " " + groupBy + " " + orderBy
      else
                ...
                case FieldDataType.OFFSET_DATETIME | FieldDataType.EPOCH_MILLIS =>
                  v match {
                    case _: OffsetDateTime | _: Timestamp =>
                      ps.setObject(index, v)
                    case l: java.lang.Long =>
                      val odt = OffsetDateTime.ofInstant(Instant.ofEpochMilli(l), Consts.ZONE_ID_Z)
  ...
  def check(queryParams: QueryParams): ParsedQueryParams[T] = {
    ...
    if (queryParams.hasSortColumns)
      queryParams.sortColumns.foreach(sc => {
        if (!supportOrderBy(parsedSearchConditions, sc.columnName))
    ...
    ParsedQueryParams(this, select, ParsedSearchConditions(parsedSearchConditions.toArray), queryParams.pagination, queryParams.sortColumns, queryParams.fields, groupByClause)
  }
...
class QueryParamsSpecBuilder[T <: AnyRef](
                                           ...
                                           val columnNames: Array[String])(implicit classTag: ClassTag[T]) {
